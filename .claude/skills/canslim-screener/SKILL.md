...
description: Screen US stocks using William O'Neil's CANSLIM growth stock methodology. Use when user requests CANSLIM stock screening, growth stock analysis, momentum stock identification, or wants to find stocks with strong earnings and price momentum following O'Neil's investment system. 当用户用中文询问时触发。支持美股/A股/港股/加密货币/贵金属多市场分析。
---
...
### A 股适配说明
当筛选 A 股标的时，需调整以下 CANSLIM 指标：
- **行业分类**: 使用申万行业分类替代 GICS
- **机构持仓**: 参考北向资金持仓、公募基金重仓数据
- **新高**: 考虑涨跌停制度影响，使用 60 日/120 日/250 日新高
- **数据源**: 使用 AKShare 获取 A 股财务和行情数据
